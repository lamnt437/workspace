...
void inOrderPrint(TreeType root, FILE *write);
    ...
    while(choice != 6){
        switch(choice){
            ...
            case 4:
                ...
                traverse(root, root, check_score);
                printf("Cay moi:\n");
                inOrder(root);
...
void inOrderPrint(TreeType root, FILE *write){
  if(root != NULL){
    inOrderPrint(root->left, write);
    fprintf(write, "%s\t%d\n", (root->data).id, (root->data).score);
    inOrderPrint(root->right, write);
  }
}
